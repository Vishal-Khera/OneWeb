function RunKuduCommand
{
	...
	do {
		...
		if($Output.ExitCode -gt 0 -or $Exception -or ($Output.Error `
			...
			-And !($Output.Error.StartsWith("warning: unable to access '/etc/mailname'")) 								<#Somehow the git throws know error, don't handle as error#> `
			))){	  	<#Somehow the push info gets returned as an error, don't handle as error#> `
