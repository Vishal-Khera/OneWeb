...
gulp.task("Publish-All-Projects",
    function(callback) {
        var deploymentRoot = config.websiteRoot;
        if (yargs.deployFolder != undefined) {
            deploymentRoot = yargs.deployFolder;
        }
        ...
        overrideParams.push("/p:publishUrl=" + deploymentRoot);
...
gulp.task("Theme-Upload",
    function(callback) {
        var server = config.websiteUrl;
        var login = config.powershellUserName;
        var password = config.powershellPassword;
        if (server != undefined && login != undefined && password != undefined) {
            exec('gulp rebuildMain --gulpfile "./src/Project/OneWeb/Theme/OneWeb/gulpfile.js" --server "' +
                server +
                '" --login "' +
                login +
                '" --password "' +
            ...
            console.log("Credentials not provided for Theme Deployment.");
        }
    });

gulp.task("Theme-Upload-DevOps",
    function(callback) {
        var server = yargs.websiteUrl;
        var login = yargs.powershellUserName;
        var password = yargs.powershellPassword;
        if (server != undefined && login != undefined && password != undefined) {
            exec('gulp rebuildMain --gulpfile "./src/Project/OneWeb/Theme/OneWeb/gulpfile.js" --server "' +
                server +
                '" --login "' +
            ...
            exec('gulp rebuildMain --gulpfile "./src/Project/OneWeb/Theme/OneWebV2/gulpfile.js" --server "' +
                server +
                '" --login "' +
                login +
                '" --password "' +
...
gulp.task("Sync-Unicorn",
    function(callback) {
        ...
        options.configurations = config.businessUnitList.join();
...
gulp.task("Auto-Publish-Assemblies",
    function() {
        ...
        var files = "/**/Sitecore.{Feature,Foundation,OneWeb}.*.{dll,pdb}";;
...
gulp.task("Package-Create-Package-Xml",
    function(callback) {
        return gulp.src(config.packageXmlBasePath).pipe(rename(config.solutionName + ".xml"))
...
gulp.task("Package-Prepare-Package-Files",
    function(callback) {
        var excludeList = [
            ...
            "!" + config.websiteRoot + "\\bin\\Sitecore.{Feature,Foundation,OneWeb,Demo,Common}*dll"
        ];
