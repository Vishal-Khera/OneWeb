...
{{
    ...
    using OneWeb.Foundation.Serialization.Models;
    ...
    [SitecoreType(TemplateId = {ConfigurationName}.{template.CodeName}.TemplateIdString)]
...
public string GetBaseInterfaces(TemplateCodeGenerationMetadata template)
{
    ...
    {
        bases.Add($"I{baseTemplate.CodeName}");
    }
    ...
    if (bases.Count == 0)
    {
        ...
        //bases.Add("OneWeb_Interface_All");
    }
...
public string RenderInterfaceFields(TemplateCodeGenerationMetadata template)
{
    ...
    {
        if (GetFieldType(field) == "Link")
        {
            localCode.AppendLine($@"
			/// <summary>{field.HelpText}</summary>
			[SitecoreField(FieldName = {ConfigurationName}.{template.CodeName}.Fields.{field.CodeName}_FieldName, UrlOptions = Glass.Mapper.Sc.Configuration.SitecoreInfoUrlOptions.LanguageEmbeddingNever)]
        ...
        else
        {
            localCode.AppendLine($@"
			/// <summary>{field.HelpText}</summary>
			[SitecoreField(FieldName = {ConfigurationName}.{template.CodeName}.Fields.{field.CodeName}_FieldName)]
			{GetFieldType(field)} {field.CodeName} {{ get; }}");
        }
    }
