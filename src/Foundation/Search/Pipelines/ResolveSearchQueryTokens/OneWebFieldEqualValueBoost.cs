...
{
    public class OneWebFieldEqualValueBoost : ResolveSearchQueryTokensProcessor
    {
        protected string TokenPart { get; } = nameof(OneWebFieldEqualValueBoost);
        ...
        public override void Process(ResolveSearchQueryTokensEventArgs args)
        {
            ...
            for (int index = 0; index < args.Models.Count; ++index)
            {
                ...
                {
                    var groups = Regex.Match(model.Value, @"^OneWebFieldEqualValueBoost\|(.*)\|(.*)").Groups;
        ...
        protected virtual SearchStringModel BuildModel(
          ...
          string operation)
        {
            return new SearchStringModel("ow_field_equal_value_boost", FormattableString.Invariant(FormattableStringFactory.Create("{0}|{1}", (object)fieldName.ToLowerInvariant(), (object)fieldValue, boost)))
            {
                Operation = operation
            };
        }

        protected override bool ContainsToken(SearchStringModel m) => Regex.Match(m.Value, @"^OneWebFieldEqualValueBoost\|(.*)").Success;
    }
}
